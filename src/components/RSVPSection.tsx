...
function FieldLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="font-body uppercase" style={{
      ...
      color: 'var(--color-gold-dark)', marginBottom: 8,
    }}>
      {children}
    </p>
  )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setState({
        ...
        guestName: data.guestName ?? form.name.trim(),
      })
    } catch {
    ...
    >
      <div style={{ maxWidth: 440, margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          <p className="font-body uppercase" style={{
            fontSize: 10, letterSpacing: '0.38em', color: 'var(--color-gold)', marginBottom: 10,
          }}>RSVP</p>
          <h2 className="font-display font-light" style={{
            fontSize: 'clamp(1.6rem, 4vw, 2.2rem)', color: 'var(--color-heading)', marginBottom: 6,
          ...
          <p className="font-display italic font-light" style={{
            fontSize: '0.95rem', color: 'var(--color-gold-dark)', opacity: 0.75,
          }}>Please respond by {WEDDING.rsvpDeadline}</p>
        </div>
        ...
        {state.phase === 'success' && (
          <div style={{ ...cardStyle, padding: '44px 32px', textAlign: 'center' }}>
            ...
            <h3 className="font-script" style={{ fontSize: '2.4rem', color: 'var(--color-gold)', lineHeight: 1.1, marginBottom: 10 }}>
              Thank You!
            </h3>
            ...
            <p className="font-display italic font-light" style={{ fontSize: '1rem', color: 'var(--color-body)', lineHeight: 1.65 }}>
              {state.attending === 'yes'
                ? <>’We can&apos;t wait to celebrate with you,<br/><strong style={{ fontStyle: 'normal' }}>{state.guestName}</strong>!</>
            ...
            <div style={{
              ...
              <span style={{
                ...
                background: state.attending === 'yes' ? '#3A6A34' : '#943030',
              }}/>
              <span className="font-body uppercase" style={{
                fontSize: 9, letterSpacing: '0.18em',
                color: state.attending === 'yes' ? '#3A6A34' : '#943030',
              }}>
        ...
        {(state.phase === 'idle' || state.phase === 'submitting' || state.phase === 'error') && (
          <form onSubmit={handleSubmit}
            style={{ ...cardStyle, padding: '32px 28px', display: 'flex', flexDirection: 'column', gap: 20 }}>
            <Corners />
                ...
                {(['yes', 'no'] as const).map(val => {
                  ...
                  return (
                    <button key={val} type="button"
                      ...
                      style={{
                        padding: '10px 0', fontFamily: 'inherit', fontSize: 9,
            ...
            {state.phase === 'error' && (
              <div style={{
                ...
                <p className="font-body" style={{ fontSize: 11, color: '#943030', lineHeight: 1.5 }}>
                  {state.message}
                </p>
                <button type="button" onClick={() => setState({ phase: 'idle' })}
                  className="font-body uppercase"
                  style={{ marginTop: 8, fontSize: 9, letterSpacing: '0.18em',
                    background: 'none', border: 'none', cursor: 'pointer',
                    color: '#943030', textDecoration: 'underline', padding: 0 }}>
                  Try again
                </button>
              </div>
            )}
            ...
            <button type="submit"
              disabled={state.phase === 'submitting' || !form.name.trim() || !form.attending}
              className="font-body uppercase"
              style={{
                ...
                boxShadow: '0 4px 18px rgba(var(--color-gold-dark-rgb),0.22)',
              ...
              {state.phase === 'submitting' ? (
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none"
                    ...
                    <path d="M7 1.5 A5.5 5.5 0 0 1 12.5 7" stroke="rgba(255,255,255,0.9)"
