...
export default function HeroSection({ wedding }: { wedding?: WeddingRow }) {
  const ringRef = useRef<SVGCircleElement>(null)

  const monogram     = wedding?.monogram      ?? 'S & M'
  const dateMedallion= wedding?.date_medallion?? ''
  const coupleNames  = wedding?.couple_names  ?? ''
  const heroLine1    = wedding?.hero_quote_line1 ?? ''
  const heroLine2    = wedding?.hero_quote_line2 ?? ''
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 flex justify-center pointer-events-none overflow-hidden">
        <svg width="700" height="80" viewBox="0 0 700 80" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ opacity: 0.35 }}>
          ...
          {[70, 160, 260, 350, 440, 540].map((x, i) => (
            <path key={i} d={`M${x} 40 Q${x + 45} 32 ${[160,260,350,440,540,630][i]} 40`}
      ...
      <div className="relative z-10 flex flex-col items-center" style={{ maxWidth: 560 }}>
        ...
        <p className="font-body uppercase mb-7" style={{ fontSize: 20, letterSpacing: '0.42em', color: 'var(--color-gold)' }}>
        ...
        <div className="relative mb-6" style={{ width: 400, height: 400 }}>
          <svg viewBox="0 0 140 140" width="300" height="400" style={{ overflow: 'visible' }}>
            <circle ref={ringRef} cx="70" cy="70" r="80"
              stroke="var(--color-gold)" strokeWidth="0.7" fill="none"
              strokeDasharray="3 6" style={{ opacity: 0.5 }}/>
            ...
            <text x="70" y="62" textAnchor="middle" fontFamily="'Great Vibes', cursive"
              fontSize="30" fill="var(--color-gold)" letterSpacing="1">
              {monogram}
            </text>
            <line x1="46" y1="72" x2="94" y2="72" stroke="var(--color-gold)" strokeWidth="0.8" opacity="0.6"/>
            <text x="70" y="85" textAnchor="middle" fontFamily="'Jost', sans-serif"
              fontSize="8" fill="var(--color-gold-dark)" letterSpacing="2.5">
              {dateMedallion}
            </text>
          </svg>
        </div>

        <h1 className="font-script mb-4" style={{
          fontSize: 'clamp(3rem, 9vw, 5.5rem)', lineHeight: 1.1,
        ...
        <div className="flex items-center justify-center mb-6" style={{ gap: 12, width: '100%', maxWidth: 320 }}>
          ...
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            {[0, 72, 144, 216, 288].map(a => (
              <ellipse key={a} cx="8" cy="8" rx="1.8" ry="4"
        ...
        {(heroLine1 || heroLine2) && (
          <blockquote className="font-display italic font-light mb-8 leading-relaxed"
        ...
        <div className="flex flex-col items-center gap-1 animate-bounce" style={{ opacity: 0.45 }}>
          ...
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--color-gold-dark)" strokeWidth="2" strokeLinecap="round">
            <line x1="12" y1="5" x2="12" y2="19"/><polyline points="5 13 12 19 19 13"/>
          </svg>
        </div>
      </div>
