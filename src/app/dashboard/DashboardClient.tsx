...
const IconWA = ({ size = 13 }: { size?: number }) => (
  ...
  return (
    <button onClick={() => { navigator.clipboard.writeText(text); setOk(true); setTimeout(() => setOk(false), 1800) }}
      style={{ display: 'inline-flex', alignItems: 'center', gap: 4,
        padding: full ? '7px 12px' : '4px 8px', fontSize: 9, letterSpacing: '0.12em',
        textTransform: 'uppercase', cursor: 'pointer', borderRadius: 1, whiteSpace: 'nowrap',
...
const WAButton = ({ onClick, label, full = false }: {
...
// ─── Guest Card (mobile) ──────────────────────────────────────
const GuestCard = ({ g, onWA, onRemove, card, mkLink }: {
  g: Guest
  onWA: (guest: Guest) => void | Promise<void>
  onRemove: (id: string) => void
  card: CSSProperties
  mkLink: (token: string) => string
}) => {
  ...
  return (
    <div style={{ ...card, padding: '14px 16px', marginBottom: 8 }}>
      ...
      {expanded && (
        <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid rgba(201,169,110,0.15)' }}>
...
— ${coupleName} 💛`
  ...
  const [waTpl,   setWaTpl]   = useState(initialTemplate || DEFAULT_TEMPLATE)
  ...
  const list = useMemo(() => guests.filter(g => {
    ...
    return (
      (g.name.toLowerCase().includes(q) || g.token.includes(q) || g.email.toLowerCase().includes(q)) &&
  ...
  // ── Create guest — calls POST /api/guests ──────────────────
  const createGuest = async () => {
    if (!nf.name.trim() || !nf.phone.trim()) {
    ...
    try {
      ...
      // Prepend new guest — server returns the created row
      setGuests(gs => [data, ...gs])
      setNf({ name: '', phone: '', email: '', passes: '1' })
      setMsg(`✓ Invitation created for ${data.name}`)
  ...
  // ── Remove guest — calls DELETE /api/guests/:id ────────────
  const removeGuest = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        // Revert on failure
        const res2 = await fetch('/api/guests')
        if (res2.ok) setGuests(await res2.json())
      }
    } catch {
      // Revert
      const res2 = await fetch('/api/guests')
  ...
  const inp: CSSProperties = {
    width: '100%', padding: '10px 13px', fontFamily: 'inherit', fontSize: 13,
    color: '#2C2C2C', background: 'rgba(201,169,110,0.05)', borderRadius: 1,
    outline: 'none', border: '1px solid rgba(201,169,110,0.26)', lineHeight: 1.5,
  }

  const lbl: CSSProperties = {
    display: 'block', fontSize: 9, letterSpacing: '0.26em', textTransform: 'uppercase',
    color: '#8B6914', marginBottom: 6,
  }

  const goldBtn: CSSProperties = {
    width: '100%', padding: '12px 22px', fontFamily: 'inherit', fontSize: 9,
      ...
      <div style={{ minHeight: '100vh', fontFamily: "'Jost',sans-serif",
        ...
        <main style={{ maxWidth: 1140, margin: '0 auto', padding: mainPad }}>
          ...
          <div className="fade-up" style={{
            ...
            gap: isMobile ? 8 : 12, marginBottom: isMobile ? 20 : 32 }}>
            {[
              ...
              <div key={label} className="fade-up" style={{ ...card,
                ...
                <Corners />
                <p style={{ fontSize: isMobile ? 8 : 9, letterSpacing: '0.2em', textTransform: 'uppercase',
            ...
            {TABS.map(({ key, label }) => (
              <button key={key} onClick={() => setTab(key)}
                style={{ padding: isMobile ? '10px 0' : '10px 22px',
                  flex: isMobile ? 1 : undefined, cursor: 'pointer', fontFamily: 'inherit',
            ...
            <div className="fade-up">
              ...
              {isMobile ? (
                <div>
                  ...
                  {list.map(g => (
                    <GuestCard key={g.id} g={g} onWA={handleWA} onRemove={removeGuest} card={card} mkLink={mkLink}/>
                  ))}
                </div>
              ) : (
                  ...
                  <div style={{ background: '#fff', borderRadius: '0 10px 10px 10px', padding: '10px 14px',
                    ...
                    <pre style={{ fontFamily: 'inherit', fontSize: 13, lineHeight: 1.75,
                      ...
                      {waTpl
                        ...
                        .replace(/{passes}/g, '2')
                        .replace(/{link}/g, `${baseUrl}/?token=tok_elena`)}
                    </pre>
                  </div>
